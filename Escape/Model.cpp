...
{
	for (GLuint i = 0; i < MAX_BONES; i++) // get location all matrices of bones
	{
...
void Model::draw(GLuint shaders_program)
{
	...
	boneTransform((double)glfwGetTime() / 1000.0f * 1000.0f, transforms);
	//boneTransform((double)glfwGetTime() / 1000.0f * 1000.0f, transforms);

	for (GLuint i = 0; i < transforms.size(); i++) // move all matrices for actual model position to shader
	{
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (GLuint i = 0; i < mesh->mNumBones; i++)
	{
		...
		{
			GLuint vertex_id = mesh->mBones[i]->mWeights[j].mVertexId; // ид вершины на которую вли€ет выбранна€ кость
...
GLuint Model::findPosition(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for (GLuint i = 0; i < p_node_anim->mNumPositionKeys - 1; i++) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
GLuint Model::findRotation(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for (GLuint i = 0; i < p_node_anim->mNumRotationKeys - 1; i++) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
GLuint Model::findScaling(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	// найти кадр который будет сразу после времени прошедшего после начала анимации
	for (GLuint i = 0; i < p_node_anim->mNumScalingKeys - 1; i++) //  Ћё„≈¬џ≈  јƒ–џ анимации
	{
...
aiVector3D Model::calcInterpolatedPosition(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	GLuint position_index = findPosition(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	GLuint next_position_index = position_index + 1; // индекс следующего ключевого кадра
...
aiQuaternion Model::calcInterpolatedRotation(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	GLuint rotation_index = findRotation(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	GLuint next_rotation_index = rotation_index + 1; // индекс следующего ключевого кадра
...
aiVector3D Model::calcInterpolatedScaling(float p_animation_time, const aiNodeAnim* p_node_anim)
{
	...
	GLuint scaling_index = findScaling(p_animation_time, p_node_anim); // вернет индекс ключевого кадра который началс€
	GLuint next_scaling_index = scaling_index + 1; // индекс следующего ключевого кадра
...
void Model::boneTransform(double time_in_sec, vector<aiMatrix4x4>& transforms)
{
	...
	time_in_ticks = time_in_ticks - GetstartTime();
	...
	for (GLuint i = 0; i < m_num_bones; i++)
	{
		...
		if (!Isforward())
		transforms[i] = identity_matrix;
	}
	if (scene->mAnimations[0]->mDuration - animation_time<0.03)
	{
		SetForward(false);
	}
}
